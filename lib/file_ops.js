 ...
 * <br>
 * * EMFILE_handler<br>
 *      * A class instance expected to have a method <b>defer</b>
 *
 * @property {Object} EMFILE_handler - A class instance expected to have a method **defer**
 * @property {Object} fs - node.js file system module
 * @memberof ExtraFileClass
 * 
 */
...
class FileOperations {
    ...
    async is_dir(path) {
        try {
            let stat = await fsPromises.lstat(path)
     ...
     * is_dirSync
     * 
     * Calls on lstatSync, and if the directory exists, 
     * this will return the result of isDirectory
     * 
     * @param {string} path 
     * @returns boolean
    ...
    /**
     * Return the dir list as an array of absolute paths to a file.
     * 
     * @param {string} path 
     * @param {object} options -- { 'just' : "dirs" } -- { 'just' : "files" } -- default everything
     */
    async dir_abs_reader(path,options) {
        try {
            let files = await fsPromises.readdir(path);
            //
            ...
            //
            if ( options.just === "files" ) {
                files = files.filter((file) => {
                    if ( this.is_dirSync(file) ) return false
                    return true
                })
            } else if (  options.just === "dirs" ) {
                files = files.filter((file) => {
                    if ( this.is_dirSync(file) ) return true
                    return false
                })
            }
            //
    ...
    /**
     * Returns an array of descriptors. Each descriptor may indicate a file or a directory.
     * 
     * For files:
     * If the suffix has been supplied, the array returned will contain only files with the given suffix.
     * If it the suffix parameter is left false, all file paths will be returned. 
     * 
     * For directories:
     * The descriptor for a directory will contain a field "subs" which will be another level of the directory tree.
     * 
     * In any case, the descript will contain the absolute path of the directory element, `abs_path`, the relative path
     * of the directory element, `rel_path`, and a field `is_file` which is true if the elemnent is a file and false if it is 
     * a directory.
     * 
     * @param {string} top_dir -- should be an absolute path to the top directory
     * @param {string|boolean} suffix 
     * @param {string|boolean} first_top -- the first top dir before recursing in dir_tree_reader 
     * @returns {Array}
     */
    async dir_tree_reader(top_dir,suffix=false,first_top = false) {
        //
        let match_array = []
        //
        try {
            ...
            let files = await fsPromises.readdir(path);
            //
            // build the next descriptors
            files = files.map(async (file) => {
                //
                let abs_path = `${path}/${file}`
                let is_file = !(this.is_dirSync(abs_path))
                if ( (typeof suffix === 'string') && is_file ) {
                    if ( !(file.includes(suffix,2)) ) {
                        return null
                    }
                }
                //
                let rel_path = first_top ? first_top : top_dir
                rel_path = abs_path.replace(top_dir,"")
                let subs = is_file ? undefined : await this.dir_tree_reader(abs_path,suffix,top_dir)
                //
                return {is_file,abs_path,rel_path,subs}
            })
            // 
            files = files.filter(fdescr => {
                return fdescr !== null
            })
            //
            return files
     ...
     * @param {object} options -- refer to the flags for node.js writeFile having to do with permissions, format, etc.
     * @returns boolean
     */
    async write_out_string(path,str,options) {
        try {
            if ( options ) {
                await fsPromises.writeFile(path,str,options)
            } else {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { return self.write_out_string(path,str,options) })
                }
            }
            console.log(path)
            console.log(e)
    ...
    async write_append_string(path,str,options) {
        try {
            if ( options ) {
                await fsPromises.appendFile(path,str,options)
            } else {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { return self.write_append_string(path,str,options) })
                }
            }
            console.log(path)
            console.log(e)
     ...
     * @param {string} path -- a path to the file that will contain the string
     * @param {object} obj -- a JSON stringifiable object
     * @param {number} spacing - indentation for JSON
     * @param {object} options -- refer to the flags for node.js writeFile having to do with permissions, format, etc.
     * @returns boolean
     */
    async write_out_pretty_json(path,obj,spacing,options) {
        try {
            let str = JSON.stringify(obj,null,spacing)
            return await this.write_out_string(path,str,options)
        } catch (e) {
            console.log(path)
            console.log(e)
    ...
    async output_string(path,str,options,top_dir) {
        try {
            ...
            return this.write_out_string(final_path,str,options)
        } catch (e) {
            console.log(path)
            console.log(e)
            return false
     ...
     * @param {string} path -- a path to the file that will contain the string
     * @param {string} str -- a string to be written
     * @param {obkect} options -- options -- refer to the flags for node.js writeFile having to do with permissions, format, etc.
     * @param {string} top_dir -- optional as starting point for the directory
     * @returns boolean
     */
    async output_append_string(path,str,options,top_dir) {
        try {
            let final_path = await this.ensure_directories(path,top_dir,true)
            if ( final_path === false ) return false
            return this.write_append_string(final_path,str,options)
        } catch (e) {
            console.log(path)
            console.log(e)
            return false
    ...
    async output_json(path,obj,options,top_dir) {
        try {
            let final_path = await this.ensure_directories(path,top_dir,true)
            if ( final_path === false ) return false
            return await this.write_out_json(final_path,obj,options)
        } catch (e) {
            console.log(path)
            console.log(e)
            return false
...
class FsExtra extends FileOperations {
    ...
    async emptyDir (dir,options,cb) {
        if ( !(await this.exists(dir)) ) {   // create an empty directory
            ...
            //
            try {
                const dir_list = await fsPromises.opendir(dir,options);
                for await (const dirent of dir_list) {
    ...
    emptyDirSync (dir,options,cb) { 
        ...
        try {
            if ( this.existsSync(dir) ) {
                ...
                while ( dir_ent ) {
                    //
                    let entry = `${dir}${sep}${dir_ent.name}`
    ...
    async createLink(srcpath, dstpath) {
        //
        let {srcStat, dstStat} = await this.getStats(srcpath, dstpath, {'dereference' : true})
    ...
    createLinkSync(srcpath, dstpath) {
        ...
        //
        if ( srcStat && dstStat && this.areIdentical(srcStat, dstStat)) return true
        //
        const containing_dir = path.dirname(dstpath)
        try {
            this.makeDirSync(containing_dir)
     ...
     * @returns boolean
     */
    async createSymlink (srcpath, dstpath) {
        //
        ...
        if ( srcStat && dstStat && srcStat.isSymbolicLink() ) {
    ...
    async outputFile(path, str, encoding = 'utf-8') {
        return await this.output_string(path,str,{ "encoding" : encoding})
    }
     ...
     * getStats
     * 
     * opts may have a field "dereference", which cause this to call fsPromises.lstat.
     * Otherwise, this calls fsPromises.stat.
     * 
     * Runs stat on two files, src and dst.
     * 
     * Returns a promise resolving to an object with two fields srcStat and destStat.
     * 
     * @param {string} src 
     * @param {string} dest 
     * @param {object} opts 
     ...
     * getStatsSync
     * 
     * opts may have a field "dereference", which cause this to call lstatSync.
     * Otherwise, this calls statSync.
     * 
     * Runs stat on two files, src and dst.
     * 
     * Returns an object with two fields srcStat and destStat.\
     * 
     * 
     * @param {string} src 
     * @param {string} dst 
     ...
     * @returns object
     */
    getStatsSync(src, dst, opts = {}) {
        try {
            if ( opts.dereference ) {
                let [srcStat, destStat] = [
                    fs.lstatSync(src,opts),fs.lstatSync(dst,opts),
                ]
                return {srcStat, destStat}
            } else {
                let [srcStat, destStat] = [
                    fs.statSync(src,opts),fs.statSync(dst,opts),
                ]
                return {srcStat, destStat}
            }
        } catch (e) {
    ...
    areIdentical(srcStat, destStat) {
        ...
        return (typeof destStat.ino !== 'undefined') &&
                (typeof destStat.dev !== 'undefined') &&
    ...
    isSrcSubdir(src, dest) {
        let abs_src = path.resolve(src)
        let abs_dst = path.resolve(dest)
        return abs_src === abs_dst || abs_src.startsWith(abs_dst + path.sep)
    }
