        ...
        try {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { self.dir_maker(path) })
                }
            }
            if ( e.code !== 'EEXIST') {
                console.error(e)
    ...
    async dir_remover(upath,recursive,force) {
        try {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { self.dir_remover(upath,recursive,force) })
                }
            }
            if ( e.code !== 'ENOENT') {
                console.error(e)
    ...
    async file_remover(path) {
        try {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { self.file_remover(path) })
                }
            }
            console.log(path)
            console.log(e)
    ...
    async write_out_string(path,str,ce_flags) {
        try {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { self.write_out_string(path,str,ce_flag) })
                }
            }
            console.log(path)
            console.log(e)
    ...
    async write_append_string(path,str,ce_flags) {
        try {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { self.write_append_string(path,str,ce_flag) })
                }
            }
            console.log(path)
            console.log(e)
    ...
    async file_copier(path_1,path_2) {
        try {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { self.file_copier(path_1,path_2) })
                }
            }
            console.log(path_1)
            console.log(e)
