...
class FileOperations {
    ...
    async ensure_directories(path,top_dir,is_file_path,app_cb) {
        //
        let sep = path.sep
        let c_path = top_dir ? `${top_dir}${sep}${path}` : path
        let file = ""
        if ( is_file_path ) {
...
class FsExtra extends FileOperations {
    ...
    moveSync (src, dest, opts) {
        ...
        //
        if ( this.different_drive(src, dest) ) {
    ...
    async createFile (file) {
        return await this.write_out_string(file,'')
    }
    ...
    // 10,11
    async symlinkPaths (srcpath, dstpath) {}
    symlinkPathsSync (srcpath, dstpath) {}

    // 12,13
    async symlinkType (srcpath, type) {}
    symlinkTypeSync (srcpath, type) {}
    ...
    outputJsonSync (file, obj, options) {
        try {
            let str = JSON.stringify(obj)
            return this.outputFileSync(final_path,str,options)
        } catch (e) {
            console.log(path)
            console.log(e)
            return false
    ...
    // 30,31
    async checkPaths(src, dest, funcName, opts) {}
    checkPathsSync(src, dest, funcName, opts) {}

    // 32, 33
    async checkParentPaths(src, srcStat, dest, funcName) {}
    checkParentPathsSync (src, srcStat, dest, funcName) {}
