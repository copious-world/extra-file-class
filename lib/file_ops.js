...
class FileOperations {
        ...
        try {
            ...
            if (  e.code == "EMFILE" ) {
                if ( this.EMFILE_handler ) {
                    let self = this
                    this.EMFILE_handler.defer(() => { return self.dir_maker(path,options) })
                }
            }
            if ( e.code !== 'EEXIST') {
                console.error(e)
    ...
    async ensure_directories(path,top_dir,is_file_path,app_cb) {
        ...
        //
        let status = this.dir_maker(c_path,{ "recursive" : true })
        if ( status ) {
            if ( is_file_path ) {
    ...
    async ensured_file_copier(path_1,path_2) {
        let status = await this.ensure_directories(path_1)
        if ( status ) {
    ...
    async file_mover(path_1,path_2) {
        ...
        if ( status ) {
            status = this.file_remover(path_1)
        }
        return status
    }
